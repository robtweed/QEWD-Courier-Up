...
function initialise() {
  ...
  for (var heading in openehr_config.headings) {
    ...
    if (typeof headingObj === 'object' && headingObj.template && headingObj.template.name) {
      ...
      getTemplate = openehr_config.paths.jumper_templates + heading + '/OpenEHR_get_template.json';
...
module.exports = function(params, callback) {
  ...
  var templateName = openehr_config.headings[heading].template.name;  // already checked it exists by now
  var headingPath = openehr_config.paths.jumper_templates + heading;
